...
async function main() {
  ...
  const logo1 = project.logoLine1 || "ML";
  const logo2 = project.logoLine2 || "Monitoring";
  const heroTitle = project.heroTitle || project.title;
  const footerTitle = project.footerProjectName || "ML monitoring pipeline";
  const footerSub =
    project.footerProjectSub ||
    "Batch validation · drift · prediction checks · reports";
