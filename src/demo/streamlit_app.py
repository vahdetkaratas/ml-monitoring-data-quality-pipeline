...
# Portfolio / demo links (change if you fork or rename the repo)
PROJECT_GITHUB_URL = "https://github.com/vahdetkaratas/ml-monitoring-data-quality-pipeline"
HTML_REPORT_REPO_PATH = "artifacts/reports/monitoring_run_report.html"
...
def _render_page_guide() -> None:
    with st.expander("What's on this page", expanded=False):
        st.markdown(
            """
- **System status & decision summary** — overall health, what failed, and suggested next steps.  
- **Run metadata & monitoring layers** — which run and files this view is based on; validation, drift, and prediction checks at a glance.  
- **Alerts & chart** — rule-level detail (sorted by severity) and the batch **churn_score** distribution.  
- **Optional details** — per-feature drift table and raw JSON; **All batches** compares every file from the same pipeline run.
            """
        )


def _render_demo_footer() -> None:
    st.divider()
    st.caption("Project & docs")
    gh = PROJECT_GITHUB_URL.rstrip("/")
    st.markdown(
        f"Source and setup: [{gh}]({gh}) · "
        f"Static HTML run summary (generated by `run_full_monitoring`): `{HTML_REPORT_REPO_PATH}` in the repo "
        "(open locally or via GitHub; this Streamlit app is the interactive viewer)."
    )
